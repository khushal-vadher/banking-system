...
//search for account existance and balance
    $search = "SELECT `account_no`, `balance` FROM `customerdata` WHERE account_no='$from_account_num' LIMIT 1";
    ...
    if ($row > 0) {
        ...
        if (($result['balance'] == 0) || ($result['balance'] <= $transfer_amount)) {            
            echo " " .$transfer_amount. $result['balance'] . $result ;
